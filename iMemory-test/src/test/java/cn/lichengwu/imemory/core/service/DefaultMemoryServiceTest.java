...
public class DefaultMemoryServiceTest {
    ...
    private void makeFixSizeMemoryService() {
        try {
            ...
            config = new Config();
            config.setMaximum(1024 * 1024).setSliceSize(200).setStorageType(StorageType.HEAP)
    ...
    private void makeMergeMemoryService() {
        try {
            ...
            config = new Config();
            config.setMaximum(1024 * 1024).setStorageType(StorageType.HEAP).setByteOrder(ByteOrder.LITTLE_ENDIAN)
